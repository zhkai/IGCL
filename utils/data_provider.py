...
datasets_path = Path("utils/datasets/")
dataset2path = {
    "MSL": {},
    "SMAP": {},
    "SMD": {},
...
def read_dataset(train_filename, test_filename, label_filename, normalize=True, file_logger=None, ratio=0.01,
    ...
    if predicted:
        ...
        for i in range(predicted_length - pre_window):
            if sum(labels[i + 1:i + 1 + pre_window, 0]) != pre_window:
                target[i, 0] = -1
            else:
                target[i, 0] = 1
        l = predicted_length - pre_window
        for i in range(l, predicted_length):
            if sum(labels[i + 1:, 0]) != predicted_length - i - 1:
                target[i, 0] = -1
            else:
                target[i, 0] = 1

        return train_data.astype(dtype='float32'), test_data.astype(dtype='float32')[:-pre_window], labels[:-pre_window], target[:-pre_window]
...
def unroll_window_3D(a):
    ...
    for channel in range(a.shape[2]):
        ...
        uni_ts = np.array([a[:, ::-1, channel].diagonal(i).mean() for i in
                           range(-a[:, ::-1, channel].shape[0] + 1, a[:, ::-1, channel].shape[1])])
